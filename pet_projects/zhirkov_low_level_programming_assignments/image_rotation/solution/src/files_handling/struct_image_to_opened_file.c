...
{  
    ...
    // size_t dest_size = sizeof(bmp_file_header);
    // size_t source_size = sizeof(header);
    //my_mem_cpy(bmp_file_header, dest_size, &header, source_size);


    if ( fwrite(&header, sizeof(uint8_t), BMP_HEADER_DATA_OFFSET, output_file ) != BMP_HEADER_DATA_OFFSET )
