...
static enum file_read_status validate_bmp_header( const struct bmp_header* const header );
enum file_read_status opened_bmp_file_to_struct_image( FILE* const input_bmp_file, struct image* const output_image );

enum file_read_status opened_file_to_struct_image( const char* source_image_filename, FILE* const input_bmp_file, struct image* const output_image )
{
    static enum file_read_status ( *opened_file_to_struct_image_handlers[2] )( FILE *const input_file, struct image *const output_image ) =
        {
        [IMAGE_FILE_FORMATS_BMP] = opened_bmp_file_to_struct_image,
...
enum file_read_status opened_bmp_file_to_struct_image( FILE* const input_bmp_file, struct image* const output_image )
{
    ...
    uint_fast8_t padding = bmp_image_width_padding_in_bytes(header.bi_width);
	...
	for ( int64_t i = header.bi_height - 1; i >= 0; i-- )
    {
